...
const collectionRequestRouter = require("./Route/CollectionRequestRoute");
const factoryWasteBinRouter = require("./Route/FactoryWasteBinRoute");
const reportsRouter = require("./Route/ReportsRoute");
...
app.get("/test-recycling-bins", (req, res) => {
  res.json({ message: "Recycling bins route is working", timestamp: new Date() });
});

app.use("/RecyclingBins", authenticateToken, recyclingBinRouter);
...
// Test route for debugging
app.get("/test-factory-waste-bin", (req, res) => {
  res.json({ message: "Factory waste bin route is working", timestamp: new Date() });
});

app.use("/FactoryWasteBin", authenticateToken, requireFactoryManager, factoryWasteBinRouter);
app.use("/CollectionRequests", authenticateToken, collectionRequestRouter);

// Test endpoint for collection requests (remove in production)
app.get("/test-collection-requests", async (req, res) => {
  try {
    const CollectionRequest = require("./Model/CollectionRequestModel");
    const RecyclingBin = require("./Model/RecyclingBinModel");
    
    const binCount = await RecyclingBin.countDocuments();
    const requestCount = await CollectionRequest.countDocuments();
    const pendingRequests = await CollectionRequest.find({ status: 'Pending' });
    const allRequests = await CollectionRequest.find();
    
    res.json({
      success: true,
      message: "Collection request system is working",
      data: {
        binCount,
        requestCount,
    ...
    res.status(500).json({
      success: false,
      message: "Error testing collection request system",
      error: error.message
    });
  }
});
...
// Get users by role
app.get("/Users/role/:role", authenticateToken, async (req, res) => {
  try {
    const User = require("./Model/UserModel");
    const { role } = req.params;
    
    // Validate role
    const validRoles = ['Admin', 'Factory Manager', 'Branch Manager', 'Driver', 'Customer', 'Fire Brigade'];
